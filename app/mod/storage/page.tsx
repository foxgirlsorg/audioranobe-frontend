  ...
  return (
    <>
      <p className={styles.hint}>
        {
          'Переключение хранилища поверх .env. Поддерживается любой S3-совместимый провайдер (AWS S3, Cloudflare R2, MinIO, Backblaze B2…). При выборе S3 конфигурация проверяется перед сохранением (тестовая запись в бакет) — сломанные параметры не будут применены. Смена хранилища не переносит уже загруженные файлы: меняется только то, куда пишутся новые и откуда отдаются.'
        }
      </p>
