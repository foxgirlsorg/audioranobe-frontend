...
/**
 * Sign in — or link an account — through Google, Discord or Telegram.
 *
 * Two very different flows behind one row of buttons:
 *
 *  - Google/Discord are ordinary OAuth. We ask the API for an authorize URL,
 *    stash the mode in sessionStorage so /auth/callback/{provider} knows what
 *    it is finishing, and hand the browser over.
 *  - Telegram has no redirect. We take a one-time code and a deep link, open
 *    the chat, and poll until the bot's webhook confirms it.
 *
 * Only providers the server actually has credentials for are rendered, so a
...
/** Marks which flow the callback page is completing. */
...
/** Providers the server has configured. Empty until /config answers. */
export function useAuthProviders(): AuthProvider[] {
  ...
  useEffect(() => {
    ...
    api<{ auth_providers?: AuthProvider[] }>('/config')
      ...
      .catch(() => {
        // optional feature — render nothing if we cannot tell
      });
    return () => {
      alive = false;
    };
...
/**
 * The buttons plus an "или" rule, for the login and register forms. Renders
 * nothing at all when no provider is configured, so the divider never appears
 * on its own above an empty space.
 */
...
export default function ProviderAuth({
  ...
  /** 'login' issues a session; 'link' attaches to the signed-in account. */
  mode: 'login' | 'link';
  /** Called after a successful link, with the refreshed identity list. */
  onLinked?: (identities: Identity[]) => void;
  /** Providers to leave out — used to hide already-linked ones. */
  hide?: AuthProvider[];
}) {
  ...
  async function startOAuth(provider: AuthProvider) {
    ...
    try {
      ...
      // The callback page is a fresh document, so the mode has to survive the
      // round trip somewhere. sessionStorage is per-tab, which is right here.
