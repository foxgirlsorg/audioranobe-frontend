...
export function ImageCropper({
  ...
  /** Optional image source — an object/data URL. */
  src?: string | null;
  /** Alias for src. */
  image?: string | null;
  /** Optional raw file — an object URL is created (and revoked) internally. */
  file?: File | Blob | null;
}) {
  ...
  // Object URL for a file passed via props.
  useEffect(() => {
    if (!file) {
      setFileUrl(null);
      return;
  ...
  // Reset state each time the dialog opens/closes.
  useEffect(() => {
    setCrop({ x: 0, y: 0 });
    setZoom(1);
    setAreaPixels(null);
        ...
        <>
          ...
          <div className={styles.actions}>
            ...
            <div className={styles.actionsRight}>
              ...
              >
                {busy ? 'Обрезка…' : 'Обрезать и сохранить'}
              </button>
            </div>
          </div>
        </>
