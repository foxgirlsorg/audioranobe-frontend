...
/**
 * Post feed for a narrator. Read-only for visitors; members get an inline
 * editor. Publishing a post notifies the narrator's subscribers (backend side).
 */
        ...
        <form className={`glass-panel ${styles.editor}`} onSubmit={save} noValidate>
          ...
          <div className={styles.field}>
            <label className={styles.label} htmlFor="post-body">
              {'Текст'} <span className={styles.optional}>{'поддерживается Markdown'}</span>
            </label>
            <textarea
              id="post-body"
              className={`textarea ${styles.bodyInput}`}
              value={body}
              maxLength={20000}
          ...
          {body.trim() ? (
            <div className={styles.preview}>
              <span className={styles.label}>{'Предпросмотр'}</span>
