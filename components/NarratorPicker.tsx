...
/** Minimal shape the picker needs; any NarratorCard satisfies it. */
...
interface Props {
  /** Currently attached narrators, in display order. */
  value: NarratorRef[];
  onChange: (narrators: NarratorRef[]) => void;
  /** Ids that cannot be removed (e.g. the last one, or ones you don't own). */
  lockedIds?: number[];
  placeholder?: string;
}

/**
 * Search across every existing narrator and attach them to a title. Mirrors
 * GenrePicker: selected narrators sit in the bar as chips, matches drop down
 * below. Deliberately searches the public catalogue rather than only the
 * signed-in user's narrators — a title owner may credit anyone.
 */
