...
interface Props {
  ...
  /**
   * When true, a query with no match offers to create the genre. Off for
   * read-only surfaces like catalog filters.
   */
  allowCreate?: boolean;
  placeholder?: string;
  /** Optional pre-loaded catalog; the picker fetches its own if omitted. */
  genres?: Genre[];
  onGenresChange?: (genres: Genre[]) => void;
}

/**
 * Search bar that doubles as the selected-genre list: chosen genres sit inside
 * the bar as removable chips and matches appear in a dropdown below. When
 * nothing matches, the bar offers to create the genre so a narrator is never
...
export default function GenrePicker({
  ...
  onChange,
  allowCreate = true,
  placeholder = 'Найти или создать жанр…',
  genres: genresProp,
  onGenresChange,
}: Props) {
  ...
  useEffect(() => {
    ...
    api<Paginated<Genre>>('/genres', { params: { per_page: 100 } })
      ...
      .catch(() => {
        // the picker still works for already-selected genres
      });
    return () => {
      alive = false;
    };
      ...
      {open && (matches.length > 0 || canCreate) ? (
        <div className={styles.dropdown} role="listbox">
          ...
          {canCreate ? (
            ...
            >
              <Plus size={13} />
              {creating ? 'Создаём…' : `Создать жанр «${query.trim()}»`}
            </button>
          ) : null}
        </div>
      ) : null}
